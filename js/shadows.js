...
jQuery(document).ready(function($) {
  $('.shadow-pull').click(function(event) {
    var is_user_sure = confirm(seravo_shadows_loc.confirm_pull);
    if ( ! is_user_sure) {
      return;
    }
    seravo_ajax_pull_shadow($(this).attr("data-shadow-name"),
      function( status ){
        if ( status == 'progress' ) {
  ...
  function seravo_ajax_pull_shadow(shadow, animate) {
    animate('progress');
    $.post(
      seravo_shadows_loc.ajaxurl,
      { type: 'POST',
        'action': 'seravo_pull_shadow',
        'pullshadow': shadow,
        'nonce': seravo_shadows_loc.ajax_nonce, },
        function( rawData ) {
          var data = JSON.parse(rawData);

  ...
  function seravo_print_plugin_table( titles, plugin, plugins_list ){
    ...
    for (var category in plugins_list) {
      if (plugins_list.hasOwnProperty(category)) {
        ...
        if (category.content.indexOf(plugin.title) != -1) {
          $( '#cruftplugins_status').append('<table><tbody id="cruftplugins_' + category.name + '"></tbody></table>');
          if (category.name == 'cache_plugins' && titles.indexOf(category.name) == -1) {
            $( '#cruftplugins_' + category.name ).prepend('<b>' + seravo_cruftplugins_loc.cache_plugins + '</b>');
          } else if (category.name == 'security_plugins' && titles.indexOf(category.name) == -1) {
            $( '#cruftplugins_' + category.name ).prepend('<b>' + seravo_cruftplugins_loc.security_plugins + '</b>');
          } else if (category.name == 'db_plugins' && titles.indexOf(category.name) == -1) {
            $( '#cruftplugins_' + category.name ).prepend('<b>' + seravo_cruftplugins_loc.db_plugins + '</b>');
          } else if (category.name == 'backup_plugins' && titles.indexOf(category.name) == -1) {
            $( '#cruftplugins_' + category.name ).prepend('<b>' + seravo_cruftplugins_loc.backup_plugins + '</b>');
          } else if (category.name == 'poor_security' && titles.indexOf(category.name) == -1) {
            $( '#cruftplugins_' + category.name ).prepend('<b>' + seravo_cruftplugins_loc.poor_security + '</b>');
          }
          inactive = false;
    ...
    if ( inactive && plugin.status == 'inactive' ) {
      if ( titles.indexOf('inactive') == -1 ) {
        $( '#cruftplugins_inactive' ).prepend('<b>' + seravo_cruftplugins_loc.inactive + '</b>');
      }
      title = 'inactive';
      inactive = false;
    }

...
if ( ! class_exists( 'Seravo_Mails_Forward_Table' ) ) {
  require_once( dirname( __FILE__ ) . '/mails-forward.php');
}
...
class Seravo_Mails_List_Table extends WP_List_Table {

  public $forwards;

  function __construct() {
    global $status, $page;
    $this->forwards = new Seravo_Mails_Forward_Table();
  ...
  function column_domain( $item ) {
    ...
    */
    $actions['view'] = sprintf('<a href="?page=%s&action=%s&domain=%s">View Forwards</a>', $_REQUEST['page'], 'view', $item['domain']);
  ...
  function process_bulk_action() {
    if ( 'delete' === $this->current_action() ) {
      ...
      $this->forwards->fetch_forward_records($_REQUEST['domain']);
    }
  }
...
$mailsTable = new Seravo_Mails_List_Table();
// Fetch, prepare, sort, and filter our data...
$mailsTable->prepare_items();

?>
<div class="wrap">

  <h1>
    <?php _e('Mails', 'seravo') ?> (beta)
  ...
  <form id="domains-filter" method="get">
    <!-- For plugins, we also need to ensure that the form posts back to our page -->
    <input type="hidden" name="page" value="<?php echo $_REQUEST['page'] ?>" />
    <!-- Now we can render the completed list table -->
    <?php $mailsTable->display() ?>
  </form>
  <?php
  if ( ! is_null($mailsTable->forwards) ) {
    $mailsTable->forwards->display();
  }
   ?>
</div>
